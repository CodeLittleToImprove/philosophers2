...
death_timing() {
  ...
  # Ensure time_to_die is a number
  if ! [[ "$time_to_die" =~ ^[0-9]+$ ]]; then
    echo "${red}[-] Test #${test_num} Error: Invalid time_to_die value: $time_to_die${reset}"
    return 1
  fi
  ...
  while read line; do
    ...
    # Ensure timestamp is extracted correctly as a number
    if ! [[ "$timestamp" =~ ^[0-9]+$ ]]; then
      echo "${red}[-] Test #${test_num} Error: Could not extract valid timestamp from line: $line${reset}"
      continue
    fi
